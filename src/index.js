const mintflowuiInfo       = require('./package.json')
...
const mainFunction = ({ addBase, addComponents, addUtilities, config }) => {
  ...
  // inject @base style 
  if (config('mintflowui.base') != false) {
    addBase(base)
  ...
  // add light themes
  if (config('mintflowui.themes') == false) {
  ...
  if(Array.isArray(config('mintflowui.themes'))){
    config('mintflowui.themes').forEach((item, index) => {
  ...
  let themeOrder = []
  if (Array.isArray( config('mintflowui.themes') )) {
    config('mintflowui.themes').forEach((theme, index) => {
